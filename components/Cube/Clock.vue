...
type ClockType = "digital" | "word";

type Language = "Nederlands" | "English" | "Deutsch" | "Français";

type AnimationInterval = 1 | 5 | 15 | 30 | 60 | undefined;

const showBackground = ref<boolean>(false);
const clockType = ref<ClockType>("digital");
const language = ref<Language>("Nederlands");
const animationInterval = ref<AnimationInterval>(undefined);
...
const cubeAppCommand = computed<string[]>(() => {
  if (clockType.value === "digital" && showBackground.value) {
    ...
    if (animationInterval.value !== undefined) {
      command.push("--animationInterval");
      command.push(String(animationInterval.value));
    }
...
async function showSmartClock(): Promise<void> {
  loading.value = true;
